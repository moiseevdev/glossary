...
var dictionaryHandler = (request, response) => {
    ...
    if (!def) {
        ...
        response.end(key + '<br>' +'was not found');
        return;
    }
    response.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
    response.end(key + '<br>' + def);
}
...
downloadDictionary('https://raw.githubusercontent.com/moiseevdev/glossary/master/dictionary.json', 'dictionary.json', (err) => {
    if (err) {
        console.log(err);
        return;
    }
